...
test('real Galley server: publish snapshot, preserve draft, copy image, then unpublish', { skip: !process.env.GALLEY_CHECKOUT }, async () => {
  ...
  try {
    ...
    await mutate('/v1/projects',{name:'Website'});
    await mutate('/v1/docs',{path:'website/intro-auto-name',content:'# INTRO\n\nMy **introduction** with a [link](https://example.com).'});
    await mutate('/v1/docs',{path:'website-other/private',content:'# INTRO\n\nPrivate intro.'});
    const doc=await mutate('/v1/docs',{path:'website/fixture',content:'# Published title\n\nThe published paragraph.'});
    ...
    const config={project:'website',introDocument:'INTRO',server:'http://127.0.0.1',publishCheckpoint:'Publish to angadjosan.com',unpublishCheckpoint:'Unpublish from angadjosan.com'};
